<div id="pdfMergeDiv" class="pkp_tab_actions" style="display: block;">			
    <ul class="pkp_workflow_decisions">
        <li>
            <a href="#" id="pdfMerge-modal-open-button" title="Merge submission files" class="pkp_controllers_linkAction pkp_linkaction_externalReview pkp_linkaction_icon_">
...
<div id="pdfMerge-modal" class="pkp_modal pkpModalWrapper" style="min-width: 100%;" tabindex="-1">
    <div class="pkp_modal_panel" role="dialog" aria-label="Merge manuscript files">
        <div class="header">{translate key="plugins.pdfMerge.modal.title"}</div><a href="#" id="pdfMerge-modal-close" class="close pkpModalCloseButton"><span
                class="pkp_screen_reader">{translate key="plugins.pdfMerge.modal.close"}</span></a>
        <div class="content">
            <div class="messageWrapper" style="margin-bottom: 10px;">
                <div id="fileListPanelSpinner" style="text-align: center; display: none;">
                    ...
                    <p style="border: 2px solid #0f0; color: #31708f; background-color: #d9edf7; border-color: #bce8f1; padding-top: 2px; padding-bottom: 2px;">{translate key="plugins.pdfMerge.modal.spinnerText"}</p>
                </div>
                <div id="successMessage" style="text-align: center; display: none; border: 2px solid #0f0; color: #3c763d; background-color: #dff0d8; border-color: #d6e9c6; padding-top: 2px; padding-bottom: 2px;">
                    <p>{translate key="plugins.pdfMerge.modal.successMessage"}</p>
                </div>
		<div id="errorMessage" style="text-align: center; display: none; border: 2px solid #f00; color: #a94442; background-color: #f2dede; border-color: #ebccd1; padding-top: 2px; padding-bottom: 2px;">
                    <p>{translate key="plugins.pdfMerge.modal.error.unknownError"}</p>
                </div>
                <div id="errorMessageForEmptyOrSingleValueList" style="text-align: center; display: none; border: 2px solid #f00; color: #a94442; background-color: #f2dede; border-color: #ebccd1; padding-top: 2px; padding-bottom: 2px;">
                        ...
                        <ul class="pkpListPanel__items" aria-live="polite">
                            {foreach from=$fileList item=file name=file}
                                <li class="pkpListPanelItem pkpListPanelItem--select">    
                                    ...
                                    <div class="pkpListPanelItem--submission__link" style="position: absolute; top: 0; right: 0; width: 4rem; height: 100%; border-left: 1px solid #eee;">
            ...
            <div id="wizardButtons" class="modal_buttons">
                <button id="startMergeButton" class="pkp_button pkp_button_primary">{translate key="plugins.pdfMerge.modal.mergeFiles"}</button>
                <button id="refreshButton" class="pkp_button pkp_button_primary" style="display: none;">{translate key="plugins.pdfMerge.modal.refreshPage"}</button>
            </div>
        </div>
    </div>	
</div>
		...
		var endpoint = "/excli/api/v1/pdfMerge/insert/{$submissionId}/{$stageId}/{$userId}"
		    ...
		    // TODO --> Remove index.php/excli from basePath while migrating to new nginx server
                    $.ajax({
                        method: "GET",
                        contentType: "application/json",
                        url: {/literal}ojsBasePath + endpoint{literal}
                    })
                    .success(function(response) { 
                        $('#successMessage').show();
                        $('#refreshButton').show();

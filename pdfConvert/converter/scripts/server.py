...
def upload_file():
    ...
    for file in fileList['files']:
        ...
        date_object = datetime.strptime(file['dateUploaded'], '%Y-%m-%d %H:%M:%S')
        try:
            db_stage_id = 2
            if file['stageId'] == 3:
                db_stage_id = 4

            filename = str(file['submissionId']) + '-' + str(file['genreId']) + '-' + str(file['fileId']) + '-' + str(file['revision']) + '-' + str(db_stage_id) + '-' + date_object.strftime("%Y%m%d")
            ...
            app.logger.info(os.path.join('/var/www/files/journals/1/articles/' + str(file['submissionId']) + '/submission', filename))
            
            full_path = '/var/www/files/journals/1/articles/' + str(file['submissionId']) + '/submission'
            ...
            raise InternalServerErrorError({'message': 'Error when converting file to PDF', 'error': e.args })
    ...
    path = '/var/www/files/journals/1/articles/' + str(submissionId) + '/submission'
